...
import { connectToDatabase, Config, RequestLog, UserKey, Payment } from "./api/_db.js";
...
async function startServer() {
  ...
  app.get("/api/pay/config", async (req, res) => {
    ...
    // Ensure FLW Plan exists if we have secret key
  ...
  app.post("/api/pay/verify", async (req, res) => {
    ...
    try {
      ...
      if (response.data.status === "success" && response.data.data.status === "successful") {
        ...
        const newUserKey = await UserKey.create({
          key,
          email,
          type,
          status: 'active',
  ...
  app.post("/api/user/dashboard", async (req, res) => {
    ...
    if (userKey.type === 'subscription' && userKey.nextPaymentDate) {
      if (new Date() > new Date(userKey.nextPaymentDate)) {
        ...
        // Remove from authorized keys if inactive
        const config = await Config.findOne({ id: 'main_config' });
        if (config) {
    ...
    // Get stats for this specific key
